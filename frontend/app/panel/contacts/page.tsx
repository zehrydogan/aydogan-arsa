...
export default function ContactsPage() {
    ...
    const handleDelete = async (contact: ContactRequest) => {
        if (confirm('Bu iletiSim talebini silmek istediGinizden emin misiniz?')) {
            try {
        ...
        <DashboardLayout>
            <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">iletiSim Talepleri</h1>
                        <p className="text-gray-600">
                            Toplam {contactsData?.total || 0} iletiSim talebi
                        </p>
                    </div>
                </div>
                ...
                {contacts.length === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                        ...
                        <h3 className="text-xl font-semibold text-gray-900 mb-2">
                            HenUz iletiSim Talebi Yok
                        </h3>
                        <p className="text-gray-600">
                            ilanlariniz iCin gelen iletiSim talepleri burada gOrUnecek.
                        </p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {contacts.map((contact: ContactRequest) => (
                            <div
                                key={contact.id}
                                className={`bg-white rounded-xl shadow-sm p-6 border-l-4 ${contact.isRead ? 'border-gray-200' : 'border-blue-500'
                                    }`}
                            >
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex items-center space-x-2 ml-4">
                                        <button
                                            ...
                                            title={selectedContact?.id === contact.id ? "Kapat" : "Detaylari GOster"}
                                        >
                                            {selectedContact?.id === contact.id ? (
                                        ...
                                        {contact.status === 'PENDING' && (
                                            <button
                                                ...
                                                title="Yanitlandi Olarak iSaretle"
